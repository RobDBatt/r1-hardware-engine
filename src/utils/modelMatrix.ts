...
export const R1_MATRIX: Record<string, ModelSpec> = {
...
export const QUANT_BITS: Record<Quantization, number> = {
...
export class ModelCalculator {
     ...
     */
    static calculateReasoningBufferGB(model: ModelSpec, tokenCount: number = 1024, bytesPerParam: number = 2): number {
     ...
     */
    static predictTPS(bandwidthGBs: number, activeParamsB: number, bitsPerWeight: number, fitsInVRAM: boolean): number {
        if (!fitsInVRAM) {
...
export class ModelMatrix {
    static getModels() {
        return Object.entries(R1_MATRIX).map(([id, model]) => {
    ...
    static getWeightVRAM(modelId: string, quant: Quantization): number {
        return R1_MATRIX[modelId]?.vramGB[quant] || 0;
    }

    static calculateMLACache(modelId: string, targetContext: number, bytesPerParam: number = 2): number {
        const model = R1_MATRIX[modelId];
        if (!model) return 0;
