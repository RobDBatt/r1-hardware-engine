...
export class HardwareScanner {
     ...
     */
    static async scan(): Promise<HardwareStats> {
     ...
     */
    static async getVRAM(): Promise<{ total: number, free: number, bandwidth: number, gpuModel: string }> {
        const platform = os.platform();
    ...
    private static async scanNvidia(): Promise<HardwareStats> {
        try {
    ...
    private static async scanMac(): Promise<HardwareStats> {
        try {
    ...
    private static _anomalyReported = false;

    /**
     * Detects unusual performance drops and encourages community reporting.
     */
    static async reportPerformance(tps: number, ttft: number) {
        if (this._anomalyReported) return;
        ...
        if (isHighEnd && (tps < 2 || ttft > 10)) {
            ...
            const selection = await vscode.window.showInformationMessage(
                "Unusual performance detected. Help us tune the heuristics for your hardware by sharing a report.",
                "Share Report"
            );
    ...
    private static getFallbackStats(): HardwareStats {
