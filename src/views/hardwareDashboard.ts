...
import { HardwareScanner, HardwareStats } from '../utils/hardwareScanner';
import { R1_MATRIX, ModelCalculator, QUANT_BITS, Quantization } from '../utils/modelMatrix';
import { LicenseService } from '../utils/licenseService';

export class HardwareDashboardProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'r1-hardware-dashboard';
    private _view?: vscode.WebviewView;

    constructor(private readonly _extensionUri: vscode.Uri) { }

    private _priority: 'context' | 'intelligence' = 'intelligence';
    ...
    private async saveConfig(modelKey: string, quant: string) {
        const config = {
            model: modelKey,
            quantization: quant,
    ...
    private _getHtmlForWebview(stats: HardwareStats) {
        const bandwidth = stats.memoryBandwidthGBs || 50;

        // Priority-based model filtering/sorting
        const sortedModels = Object.entries(R1_MATRIX).sort(([aK, aM], [bK, bM]) => {
        ...
        const models = sortedModels.map(([key, model]) => {
            ...
            const quant = this._priority === 'context' ? Quantization.Q8_0 : Quantization.Q4_K_M;
            const targetVram = model.vramGB[quant];
            const fits = stats.vramAvailableGB >= targetVram;
            const tps = ModelCalculator.predictTPS(
                bandwidth,
                activeParams,
                QUANT_BITS[quant],
                fits
            ...
            <body>
                ...
                <div class="toggle-group">
                    ...
                    <button class="toggle-btn ${this._priority === 'intelligence' ? 'active' : ''}" onclick="setPriority('intelligence')">
                        ...
                        ${!LicenseService.isPro() ? '<span style="font-size: 0.7em;">🔒</span>' : ''}
                    </button>
                </div>
